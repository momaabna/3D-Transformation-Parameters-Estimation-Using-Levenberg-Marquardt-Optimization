...
H =matlabFunction(H);
%% Initialization 
solold = [5*pi,5*pi,5*pi,1000000,1000000,1000000,1000]';
ftol = 1E-10; 
gtol =1E-10;
lamda=1;
iter=0;
...
while 1
   ...
   if abs(sd(1))>1E-9 | abs(sd(2))>1E-9 |abs(sd(3))>1E-9 | abs(sd(4))>1E-7|abs(sd(5))>1E-7|abs(sd(6))>1E-7| abs(sd(7))>1E-12 %abs(fnew-fold)>ftol | sum(gvf.^2) >gtol %
       fprintf('Iter : %.0f,F Loss:%.10f , diff:%.10f \n ',iter,fold,sum(abs(sd)))
       %disp(sd')
       res =[res;[iter,sum(abs(sd)),fold]];
   else
       disp('Final Solution' )
       disp(solnew);
       disp(fnew);
